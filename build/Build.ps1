...
task Compile -depends Version, Clean { 
	...
	$solutionPath = "$solutionDir\Enexure.Fire.sln"
	Invoke-MsBuild $solutionPath -MSBuildProperties @{ Configuration = "Release" }
}

task Version -depends Clean {

	$versionSourceFile = "$solutionDir\src\Enexure.Fire\Version.json"
	...
	$projectDir = "$solutionDir\src\Enexure.Fire\Properties"
